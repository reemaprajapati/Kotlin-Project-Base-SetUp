...
import com.yipl.labelstep.util.Utils
...
class Repository @Inject constructor(private val context: Context, private val apiService: ApiService, private val dao: DatabaseDao) {
    ...
    var token = "Bearer " + "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOjI2LCJpc3MiOiJodHRwOi8vbHMuc3RhZ2luZy55aXBsLmNvbS5ucC8vYXBpL2xvZ2luIiwiaWF0IjoxNTI2MzYyODgzLCJleHAiOjE1MjY0NDkyODMsIm5iZiI6MTUyNjM2Mjg4MywianRpIjoiUjZJNFVyZWNrUUxlRDZ3ZiJ9.ugLzN92A6DuZ9hAxZPNOUQacQCJM-dGw7qVCQySyxAU";


    fun getStandards(): Observable<StandardsEntity> {
        if (isThereInternetConnection()) {
            return apiService.getStandards(token)
    ...
    fun getSuppliers(): Observable<SupplierEntity> {
        if (isThereInternetConnection()) {
            return apiService.getSuppliers(token)
                    .doOnNext { supplierEntity: SupplierEntity? ->
                        run {
                            for (entity in supplierEntity!!.data) {
                                dao.insertSupplier(entity)
                                for (upComingAudit in entity.upcomingAuditList) {
                                    upComingAudit.supplierId = entity.id
                                    upComingAudit.idS= Utils.getUniqueId(26,entityId = entity.id.toLong())
                                    dao.insertUpcomingAudits(upComingAudit)
                                }
                            }
